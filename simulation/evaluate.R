...
if (length(single_results) > 0 && length(multi_results) > 0) {
  ...
  single_fp <- mean(sapply(single_results, function(x) x$unexpressed$fp_rate), na.rm = TRUE)
  multi_fp <- mean(sapply(multi_results, function(x) x$unexpressed$fp_rate), na.rm = TRUE)
  single_max <- max(sapply(single_results, function(x) x$unexpressed$max_tpm), na.rm = TRUE)
  multi_max <- max(sapply(multi_results, function(x) x$unexpressed$max_tpm), na.rm = TRUE)
